...
simulate () {
        this.mctsTimer = setInterval(this.doMCTS(), this.mctsIntvl);               
}
...
doMCTS () {  // does one cycle .. timer calls it many times
    ...
    if ( this.count > this.maxCount-1 ) { // Done with computations
	 ...
	 // We plot paths for the last iteration 
       ...
       if (((this.count+1)% (this.maxCount/this.pdfTraces)) == 0) {
...
getBeta (count) {
  ...
  let b = beta.min + (beta.max - beta.min)
           * sCurve(beta.anneal, count / (this.maxCount-1)); 
  return(b);
 }
...
class pathPlot {
  ...
  start () {
        this.timer = setInterval(this.plot, this.intval) ;
  } // end start
  ...
  plot () {
       ...
       for (let i=0 ; i < this.team.robots.length ; i++) {
         let vec = this.getMoves(this.team.robots[i], this.count)
